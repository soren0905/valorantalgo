...
def main():
    ...
    combined_stats_path = os.path.join(project_root, 'data', 'JSON', 'map_stats', 'combined_map_stats.json')  # Updated path
    output_json_path = os.path.join(project_root, 'data', 'JSON', 'stats.json')
    output_csv_path = os.path.join(project_root, 'data', 'CSV', 'stats.csv')

    # Check if files exist before proceeding
    if not os.path.exists(upcoming_players_path):
        print(f"File not found: {upcoming_players_path}")
        return
    if not os.path.exists(combined_stats_path):
        print(f"File not found: {combined_stats_path}")
        return
